...
- Python 3.12 (the project uses a virtual environment)
- pip and virtualenv (or `python -m venv`)
- SQLite (project includes `db.sqlite3` by default)
...
Visit `http://127.0.0.1:8000/` to view the app (login page / home).

Notes about the `date` field:
- The `Food.date` field is configured with `auto_now_add=True`, so new food entries automatically receive the creation date. If you change the model you will need to run migrations as above.
